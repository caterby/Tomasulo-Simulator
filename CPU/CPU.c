...
void initializeCPU () {
	int i;
	int numberOfIntRenameRegisters = 8;
	int numberOfFpRenameRegisters = 8;
...
//initialize renaming interger registers
        cpu -> intRenamingRegister = (INTReg **) malloc (sizeof(INTReg *) * numberOfIntRenameRegisters);

        for (i = 0; i < numberOfIntRenameRegisters; i++) {
                cpu -> intRenamingRegister [i] = (INTReg *) malloc (sizeof(INTReg));
                cpu -> intRenamingRegister [i] -> data = 0;
                cpu -> intRenamingRegister [i] -> intResult = 0;
        }

//initialize renaming float registers
cpu -> fpRenamingRegister = (FPReg **) malloc (sizeof(FPReg *) * numberOfFpRenameRegisters);

for (i = 0; i < numberOfFpRenameRegisters; i++) {
        cpu -> fpRenamingRegister [i] = (FPReg *) malloc (sizeof(FPReg));
        cpu -> fpRenamingRegister [i] -> data = 0;
        cpu -> fpRenamingRegister [i] -> fpResult = 0;
}
